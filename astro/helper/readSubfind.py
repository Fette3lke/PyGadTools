...
def readSubfind(self, snapnum = None):
  if snapnum == None:
    snapnum = int(self.filename[-3:])
  path = os.path.dirname(self.filename)
  if not path:
    path = "./"
  self.subfind = subfind_catalog(path, snapnum)
  self.subfind.read_ids()
  

def getSubHalo(self, n, snapnum = None, **kwargs):
  if not self.subfind:    
    self.readSubfind(snapnum)
      
  if n == len( self.subfind.sub_offset ):
    cut_ids = self.subfind.ids[ self.subfind.sub_offset[n] : ]
  else:    
    cut_ids = self.subfind.ids[ self.subfind.sub_offset[n] : self.subfind.sub_offset[n+1]]
    
  indices = np.in1d( self.ids, cut_ids )
  
  docopy = kwargs.pop('docopy', True)
  subhalo = self.slice(indices, assign=False, docopy=docopy)

  subhalo.pos -= self.subfind.sub_pos[n] * self.head.time / self.head.hubble
  subhalo.vel -= self.subfind.sub_vel[n] * np.sqrt(self.head.time) 

  subhalo.assign_names()
  return subhalo
